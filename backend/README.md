...
- **`Video not found, private, or unavailable` on a video that works locally**: this almost always means YouTube is blocking the host's IP with "Sign in to confirm you're not a bot" — check the web service's logs for the actual `yt-dlp` error (logged, but deliberately not sent to the client). Common on cloud hosts (Railway, Render, etc.), rare on a residential/local IP. Fix: set `YT_DLP_COOKIES` to the full contents of a Netscape-format `cookies.txt` exported from a real, logged-in YouTube session:
  ...
  **Root cause found and fixed** (after an earlier attempt at this using a `tv_embedded` player client turned out to be a no-op — confirmed live via `yt-dlp`'s own warning, `Skipping unsupported client "tv_embedded"`, meaning that whole earlier attempt was silently just testing the default "web" client). What actually works, confirmed live end-to-end (`fetch_metadata` → `download_audio` → `transcribe`), is three things together:

  1. **`player_client=tv_simply`** (`app/services/youtube.py`) — unlike "web", this client doesn't need a login/cookies at all, and explicitly requires a PO (proof-of-origin) token for its media formats, which is what actually engages the fix below (confirmed live: with "web", `yt-dlp` never even attempts to request a PO token, because "web"'s bot-check happens at a stage `yt-dlp`'s own policy doesn't think needs one — this is why cookies alone never helped, no matter how correctly they were configured).
  2. **A running PO-token provider** — deploy [`brainicism/bgutil-ytdlp-pot-provider`](https://github.com/Brainicism/bgutil-ytdlp-pot-provider) as its own service (a prebuilt Docker image, deployable directly — e.g. on Railway: "New" → "Empty Service" → set the Docker image, no repo needed) exposing port `4416`. Set `POT_PROVIDER_BASE_URL` on **both** the web and worker services to its internal URL (e.g. `http://<service-name>.railway.internal:4416` on Railway), and redeploy both. `app/services/youtube.py` passes this to `yt-dlp` via the `youtubepot-bgutilhttp` extractor arg once the env var is set.
  3. **A JS runtime (Deno) + `yt-dlp`'s remote EJS component**, to solve YouTube's "n challenge" signature obfuscation — without this, formats silently degrade to images-only even with a valid PO token. `nixpacks.toml` in this directory adds `deno` as a Nix package so Railway's build installs it automatically (verify this actually takes effect after your first deploy — if Nixpacks doesn't pick it up as expected, switching to a Dockerfile that installs Deno directly is the fallback). `app/services/youtube.py` sets `remote_components: {"ejs:github"}`, which lets `yt-dlp` download the actual solver script from GitHub releases at runtime (no extra pip/npm package needed).

  Cookies (`YT_DLP_COOKIES`) are kept wired in as a harmless fallback but aren't used by `tv_simply` itself — the fix above doesn't depend on them.

  Also fixed alongside this: the audio format selector previously picked the *highest*-bitrate audio-only stream unconditionally, which for some videos (confirmed live: a demo video with a 387kbps audio track) produced a 30MB+ file exceeding Groq's 25MB cap. It's now capped at 128kbps first (plenty for speech transcription accuracy), falling back to whatever's available if nothing fits.
